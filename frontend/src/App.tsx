    ...
    <DesignCanvas>
      ...
      >
        {phone(
          '01 · Splash',
          '01',
          '앱 진입 · 캐릭터 컨셉(밤에 깨어남) 첫 노출. 자동 진행 1.5s.',
          '★ 밤 하늘 + 깨어난 이음이 — 컨셉을 첫 1.5초에 전달',
          <S01_Splash />,
        )}
        {phone(
          '02 · Welcome',
          '02',
          '1인 가구 가치 제안. [시작하기] 탭 → 03으로.',
          '★ 온보딩 첫인상 — status bar에서 시간대(아침)임을 암시',
          <S02_Welcome />,
        )}
        {phone(
          '03 · Privacy',
          '03',
          '로컬 우선 저장·익명 분석·언제든 삭제 — 신뢰 약속.',
          '★ 일기 = 가장 사적인 데이터 — 신뢰를 먼저 약속하고 시작',
          <S03_Privacy />,
        )}
        {phone(
          '04 · 캐릭터 생성',
          '04',
          '이름·털색·성격 선택. 성격은 말투에 영향.',
          '★ 캐릭터 = 내 분신 — 작은 커스텀이 큰 애착으로',
          <S04_CreateCat />,
        )}
        {phone(
          '05 · 첫 만남 (밤)',
          '05',
          '이음이 첫 대사 — 밤 모드 컨셉을 첫 화면에서 학습.',
          '★ 컨셉 — 밤에만 깨어나는 캐릭터를 첫 화면부터 학습시킴',
          <S05_FirstMeet />,
        )}
      </DCSection>
      ...
      >
        {phone(
          '06 · 홈 (낮)',
          '06',
          '센터 [홈] 탭 진입 — 낮엔 이음이 수면. 데일리 체크 + AI 비서 진입점.',
          '★ 낮 모드 — 고양이 자는 중, 회고 CTA 없음. 데일리 체크가 메인',
          <S06_HomeDay />,
        )}
        {phone(
          '08 · 데일리 체크',
          '08',
          '홈 카드 탭 → 식사·수면·운동·물·햇볕 5체크. 회고 자동 메모.',
          '★ 5개 = 부담 적게, 점수보다 흔적',
          <S08_DailyCheck />,
        )}
        {phone(
          '09 · AI 비서 챗',
          '09',
          '홈 카드 탭 → 낮 자유 대화. 답변은 밤 회고로 자동 인계.',
          '★ 낮은 AI 비서 / 밤은 캐릭터 회고 — 톤이 다름',
          <S09_AIChat />,
        )}
      </DCSection>
      ...
      >
        {phone(
          '07 · 홈 (밤)',
          '07',
          '저녁 시간대 자동 전환. 회고 CTA가 hero — 깨어난 이음이가 부른다.',
          '★ 밤 모드 — 회고 CTA가 hero, 깨어난 캐릭터가 부른다',
          <S07_HomeNight />,
        )}
        {phone(
          '10 · 회고 시작',
          '10',
          '[시작하기] → 낮 메모 인계 + 모드 선택(챗/짧게/보이스).',
          '★ NEW — 낮 동안 적어둔 메모가 회고를 더 똑똑하게',
          <S10_RecapStart />,
        )}
        {phone(
          '11 · ChatDiary (5턴)',
          '11',
          '이음이와 5턴 대화. AI가 행동 제안으로 마무리.',
          '★ AI가 회고를 "행동 제안"으로 마무리 — 챗봇 ≠ 일기',
          <S11_ChatDiary />,
        )}
        {phone(
          '12 · 감정 분석 · 일기',
          '12',
          '5턴 결과 → 감정 분포 + AI 일기 초안 + 내일 한 가지.',
          '★ 감정 분석 + AI 일기 초안 + 내일 1가지 — 한 화면에 통합',
          <S12_MoodFinalize />,
        )}
        {phone(
          '13 · 보상 모달',
          '13',
          '저장 직후 — 포인트/스트릭/새 아이템. 키우기로 자연 유도.',
          '★ 회고 → 즉시 보상 → 키우기로 자연스러운 동선',
          <S13_Reward />,
        )}
      </DCSection>
      ...
      >
        {phone(
          '14 · 달력',
          '14',
          '좌측 [달력] 탭 → 월별 이모지 캘린더. 셀 탭 → 일기 디테일.',
          '★ 이모지 셀 = 감정 / 빈 동그라미 = 기록 없음',
          <S14_Calendar />,
        )}
        {phone(
          '15 · 일기 디테일',
          '15',
          '감정 분포·일기 본문·키워드·그날의 체크·내일 한 가지 이행 여부.',
          '★ 일기 = 회고 결과의 영구 보관 페이지',
          <S15_DiaryDetail />,
        )}
      </DCSection>
      ...
      >
        {phone(
          '16 · 통계',
          '16',
          '[통계] 탭 → 주/월 KPI · 요일별 작성 · 감정 분포 · 라이프스타일.',
          '★ D30 북극성 — 주 3회 이상 작성 60%',
          <S16_Stats />,
        )}
        {phone(
          '17 · 인사이트',
          '17',
          '[인사이트] 탭 → AI가 찾은 이번 주 패턴 → 추천 루틴 → 알람 추가.',
          '★ 패턴 → 행동 제안 → 알람화 — 인사이트가 "행동"으로',
          <S17_Insights />,
        )}
      </DCSection>
      ...
      >
        {phone(
          '18 · 이음이 방 (밤)',
          '18',
          '[키우기] 탭 → 친밀도/배부름/활력 + 먹이주기·옷장·놀이·방꾸미기 4액션.',
          '★ 키우기 = 회고 보상의 시각화 — 방·옷·먹이',
          <S18_CatRoom />,
        )}
        {phone(
          '19 · 인벤토리 · 옷장',
          '19',
          '모은 아이템·먹이·잠금 보상. 잠금 = 다음 회고 동기.',
          '★ 잠금 아이템 = 다음 회고 동기 / "입는중" = 현재 적용',
          <S19_Inventory />,
        )}
        {phone(
          '20 · 주간 리포트',
          '20',
          '매주 월요일 푸시 — 한 줄 요약 + 숫자 그리드 + 이번 주 이야기 + 카드 저장.',
          '★ 주간 리포트 = 시리얼 컨텐츠 · 매주 월요일 푸시로 재방문',
          <S20_Report />,
        )}
      </DCSection>
    </DesignCanvas>
...
export const App = () => {
  ...
  useEffect(() => {
    const onHash = () => setMode(window.location.hash === '#design' ? 'design' : 'app');
