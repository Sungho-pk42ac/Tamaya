...
const PALETTES: Record<Palette, { label: string; swatch: string[]; vars: Record<string, string> }> =
  {
    coffee: {
      label: 'Coffee Lounge',
...
export const TweaksPanel = () => {
  const initial = load() ?? {
    palette: 'coffee' as Palette,
    vibe: 'crisp' as Vibe,
    density: 'detailed' as Density,
  };
      ...
      {open && (
        ...
        >
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            {(Object.entries(PALETTES) as [Palette, (typeof PALETTES)[Palette]][]).map(([k, p]) => (
              <button
                key={k}
